...
const Delete: FastifyPluginAsync = async (fastify: FastifyInstance) => {
  ...
  fastify.delete('/:organizationId/:projectId/:filename', async (request, reply) => {
    ...
    fastify.delete('/:organizationId/:projectId/user/:filename', async (request, reply) => {
	const { organizationId, projectId, filename } = request.params as { organizationId: string, projectId: string, filename: string};
	const userId = getUserIdFromJWT(request, reply, fastify)
	if (!userId || Number.isNaN(userId)) {
		reply.code(401)
	...
	const filePath = path.join('uploads', organizationId, projectId, userId.toString(), filename)
	if (organizationId) {
		if (projectId) {
			// ========= CONTROLLO CHE L'ORGANIZZAZIONE ESISTA ========
			const org = await fastify.prisma.organization.findUnique({
	...
	try {
		await fs.promises.unlink(filePath)
		const dirPath = path.join("uploads", organizationId, projectId, userId.toString())
		const dir = await fs.promises.readdir(dirPath)
		if (dir.length === 0){
			await fs.promises.rmdir(dirPath)
			const orgPath = path.join("uploads", organizationId)
