...
//     }
//     })
// }import { RoleName, Status, FriendshipStatus, Priority } from "@prisma/client";
    ...
    try {
        ...
        const u = await prisma.user.create({
            ...
            surname: pick(lastNames),
            email: randomEmail(i),
            phone: maybeNull(randomPhone(), 0.8), // phone ora è String?
            city: maybeNull(pick(cities), 0.7),
            address: maybeNull(`Via Test ${randInt(1, 200)}`, 0.6),
            cap: maybeNull(String(randInt(10000, 99999)), 0.6),
            state: maybeNull('IT', 0.6),
        ...
        // 8) TASKS + TASK PARTICIPANTS (con priority + dueDate)
        const tasks: any[] = []
