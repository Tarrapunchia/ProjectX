...
export default function ModifyEventModal({ event, onClose }: { event: any, onClose: (updatedData?: any) => void }) 
{
        ...
        const [formData, setFormData] = useState({
            ...
            description: event.description || "", // Qui ora la descrizione c'è!
            dueDate: formatInitialDate(event.rawStart), // <--- USIAMO rawStart!
        });
    ...
    // La funzione removeFriend ora gestisce sia i partecipanti vecchi che quelli nuovi
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // Utilizziamo putter al posto di poster
        ...
        if (res?.success) {
            ...
            alert("Errore: " + (res?.data.error ||"Impossibile aggiornare l'evento"));
        }
        setLoading(false);
    };
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[200] flex items-center justify-center p-4">
            <div className="bg-side-bg-color border border-overlay-border-color w-full max-w-md rounded-2xl shadow-2xl p-6">
                <div className="flex justify-between items-center mb-6">
                    <h3 className="text-xl font-bold text-text-main">Modify Event</h3>
