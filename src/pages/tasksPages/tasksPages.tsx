...
export default function TasksLibrary() {
    ...
    if (isLoading) return <div className="w-full h-full flex items-center justify-center text-gray-400">Loading tasks...</div>;

    return (
        <div className="flex flex-col h-full w-full p-6 bg-main-bg-color overflow-y-auto custom-scrollbar">
            <h1 className="text-2xl font-bold text-text-main mb-6 shrink-0">Tasks Library</h1>
            <div className="flex flex-wrap gap-4 mb-8 shrink-0" ref={containerRef}>
                <div className="relative">
                    <button onClick={() => setOpenDropdown(openDropdown === 'projects' ? null : 'projects')} className={`flex items-center gap-2 bg-side-bg-color border text-sm font-medium rounded-xl px-4 py-2.5 transition-colors cursor-pointer shadow-sm ${openDropdown === 'projects' ? 'border-text-main text-text-main' : 'border-overlay-border-color text-text-main hover:border-text-main'}`}>
                        <Filter size={14} /> <span>Filter Projects ({selectedProjects.length})</span>
                    </button>
                    {openDropdown === 'projects' && (
                        <div className="absolute top-full mt-2 left-0 w-64 bg-side-bg-color border border-overlay-border-color rounded-xl shadow-2xl z-50 overflow-hidden flex flex-col">
                            <div className="p-2 border-b border-overlay-border-color flex flex-col gap-1">
                                <label className="flex items-center gap-3 text-sm text-text-main cursor-pointer px-3 py-2 hover:bg-main-bg-color rounded-lg"><input type="checkbox" checked={selectedProjects.length === projectsList.length && projectsList.length > 0} onChange={toggleAllProjects} className="accent-owner-color w-4 h-4 cursor-pointer" /> <span className="font-bold text-text-main">(Select All)</span></label>
                                <label className="flex items-center gap-3 text-sm text-text-main cursor-pointer px-3 py-2 hover:bg-main-bg-color rounded-lg"><input type="checkbox" checked={onlyOwned} onChange={toggleOnlyOwned} className="accent-owner-color w-4 h-4 cursor-pointer" /> <span className="flex items-center gap-1.5 font-bold text-text-main"><ShieldCheck size={14} /> Only Owned Projects</span></label>
                            </div>
                    ...
                    return (
                        <div key={`${task.projectId}-${task.id}`} className="flex flex-col md:flex-row md:items-center justify-between p-5 bg-side-bg-color border border-overlay-border-color rounded-xl shadow-sm hover:border-text-main transition-all gap-4 group">
                            <div className="flex-1 min-w-0">
                                ...
                                <p className="text-sm text-zinc-400 line-clamp-2">{task.description || "No description."}</p>
                            </div>
                            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 shrink-0">
                                <div className="flex flex-col gap-1.5 text-[11px] text-zinc-500 border-l border-overlay-border-color pl-4"><div className="flex items-center gap-1.5"><Calendar size={12} /><span>Created: {formatDate(task.createdAt)}</span></div><div className="flex items-center gap-1.5"><AlertCircle size={12} className={task.dueDate && new Date(task.dueDate) < new Date() && !['COMPLETED', 'CLOSED'].includes(task.status) ? "text-red-400" : ""} /><span className={task.dueDate && new Date(task.dueDate) < new Date() && !['COMPLETED', 'CLOSED'].includes(task.status) ? "text-red-400 font-bold" : ""}>Due: {formatDate(task.dueDate)}</span></div></div>
                                <div className="relative"><div className="absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none text-zinc-400"><StatusIcon size={14} /></div><select value={task.status} onChange={(e) => handleStatusChange(task.projectId, task.id, e.target.value)} disabled={task.roleId !== ROLE_OWNER} className={`appearance-none bg-main-bg-color border border-overlay-border-color text-text-main text-xs font-bold rounded-lg pl-9 pr-10 py-2.5 focus:outline-none focus:border-text-main ${task.roleId === ROLE_OWNER ? 'cursor-pointer hover:border-text-main' : 'cursor-not-allowed opacity-50'}`}>{Object.keys(statusConfig).map(s => <option key={s} value={s}>{statusConfig[s].label}</option>)}</select><div className="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none text-zinc-400"><ArrowUpDown size={12} /></div></div>
                            </div>
                        </div>
                    );
                })}
                {processedTasks.length === 0 && <div className="text-center py-20 text-zinc-500 bg-side-bg-color border border-overlay-border-color rounded-2xl border-dashed"><p className="text-lg">No tasks found matching your filters.</p></div>}
            </div>
        </div>
    );
}