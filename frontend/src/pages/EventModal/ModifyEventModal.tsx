...
export default function ModifyEventModal({ event, onClose }: { event: any, onClose: (updatedData?: any) => void }) 
{
    ...
    const { loadCalendar, friends, activeUser } = useWebSocket();
    const [loading, setLoading] = useState(false);
    const [selectedFriends, setSelectedFriends] = useState<number[]>(event.participants || []);
    const [searchTerm, setSearchTerm] = useState("");
    const [startDate, setStartDate] = useState<Date | null>(event.rawStart ? new Date(event.rawStart) : new Date());

    const cleanName = event.name ? event.name.replace(/^(📅|🧩)\s*/, '') : '';
    const cleanId = event.id ? String(event.id).replace(/^(event:|task:)/, '') : '';
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const res = await helpers.putter(`/api/v1/events/${cleanId}`, payload);

        if (res?.success) {
            await loadCalendar();
            onClose(payload);
        } else {
            alert(t("modify_event_modal.error") + (res?.data.error || "Impossibile aggiornare l'evento"));
        }
        setLoading(false);
    };
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[200] flex items-center justify-center p-4">
            <div className="bg-side-bg-color border border-overlay-border-color w-full max-w-md rounded-2xl shadow-2xl p-6">
                <div className="flex justify-between items-center mb-6">
                    <h3 className="text-xl font-bold text-text-main">{t("modify_event_modal.title")}</h3>
                    <button type="button" onClick={() => onClose()} className="text-text-main rounded-full hover:scale-105 hover:border hover:border-text-main cursor-pointer">
                        <X size={20} />
                    </button>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-4 overflow-y-auto pr-2 custom-scrollbar">
                    <div className="space-y-3">
                        <input 
                            required
                            placeholder={t("modify_event_modal.placeholder_title")}
                            value={formData.name}
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            ...
                            >
                                <option value="NONE">{t("modify_event_modal.type_none")}</option>
                                <option value="MEETING">{t("modify_event_modal.type_meeting")}</option>
                                <option value="CALL">{t("modify_event_modal.type_call")}</option>
                                <option value="CONFERENCE">{t("modify_event_modal.type_conference")}</option>
                        ...
                        <textarea 
                            placeholder={t("modify_event_modal.description")}
                    ...
                    <div className="flex flex-col gap-2 mt-2">
                        <div className="relative">
                            <div className="relative">
                                ...
                                <input 
                                    type="text"
                                    placeholder={t("modify_event_modal.search_friend")}
                                    value={searchTerm}
                                ...
                                <div className="absolute top-full left-0 w-full mt-1 bg-side-bg-color border border-overlay-border-color rounded-xl shadow-2xl z-50 overflow-hidden max-h-40 overflow-y-auto custom-scrollbar">
                                    {searchResults.length > 0 ? (
                                        searchResults.map(friend => (
                                            <div key={friend.id} onClick={() => addFriend(friend.id)} className="p-3 hover:bg-owner-color/10 cursor-pointer text-xs text-text-main flex justify-between items-center border-b border-overlay-border-color">
                                        ...
                                        <div className="p-4 text-center text-[10px] text-zinc-500 italic">{t("modify_event_modal.no_friends")}</div>
                                    )}
                                </div>
                            )}
                        </div>
                        ...
                        <div className="flex flex-col gap-2">
                            <label className="text-[10px] uppercase font-bold text-zinc-500 ml-1 tracking-widest">
                                {t("modify_event_modal.participants", { count: selectedFriends.length })}
                            </label>
                            ...
                            <div className="min-h-[50px] p-2 rounded-xl bg-main-bg-color/30 border border-dashed border-overlay-border-color flex flex-wrap gap-2 items-center">
                                {selectedFriends.filter(id => id !== activeUser?.id).length === 0 ? (
                                    <span className="text-[10px] text-zinc-600 italic ml-2">{t("modify_event_modal.no_participants")}</span>
                                ) : (
                                    selectedFriends.filter(id => id !== activeUser?.id).map(id => {
                                        ...
                                        return (
                                            <div key={id} className="flex items-center gap-2 bg-owner-color/20 border border-owner-color/30 pl-1 pr-2 py-1 rounded-lg animate-in fade-in zoom-in duration-200">
                                                <div className="w-8 h-8 rounded-full bg-side-bg-color overflow-hidden shrink-0 border border-overlay-border-color">
                                                    <img src={`${CONSTS.BE}/api/v1/users/${friend?.id}/avatar`} alt="avatar" className="w-full h-full object-cover" />
                                                </div>
                                                <span className="text-[11px] text-text-main font-medium">{friend?.name}</span>
                                                <button type="button" onClick={() => removeFriend(id)} className="text-zinc-500 hover:text-red-500 transition-colors cursor-pointer ml-1"><X size={12} /></button>
                                            </div>
                                        );
                                    })
                                )}
                    ...
                    <div className="flex justify-center mt-4">
                        <button disabled={loading} className="w-[50%] py-2 bg-owner-color text-white rounded-xl font-bold hover:scale-[1.02] active:scale-95 transition-all shadow-lg disabled:opacity-50 cursor-pointer">
                            {loading ? t("modify_event_modal.updating") : t("modify_event_modal.update")}
                        </button>
                    </div>
                </form>
            </div>
